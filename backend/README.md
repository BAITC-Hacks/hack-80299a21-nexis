...
Тесты используют синтетический транспорт и временные SQLite. Live smoke отдельно читает настоящий API с .env и проверяет только временную локальную корзину; записей у партнёра нет. Работа настоящей модели, нагрузка и полный браузерный сценарий оцениваются отдельно.
